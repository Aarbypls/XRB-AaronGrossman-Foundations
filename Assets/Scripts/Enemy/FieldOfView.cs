    ...
    void Update()
    {
        ...
        foreach (Collider target in targetsInViewRadius)
        {
            ...
            if (Vector3.Angle(transform.forward, directionToTarget) < _viewAngle)
            {
                ...
                float distanceToTargetHead = Vector3.Distance(headPos, targetHeadPos);

                if (Physics.Raycast(headPos, directionToTargetHead, distanceToTargetHead, _blockingLayers))
                {
                    continue;
                }
